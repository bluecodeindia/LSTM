...
def build_skeleton_model(n_timesteps, n_features, n_outputs):
    ...
    '''VANILA LSTM'''
    # model.add(LSTM(200, return_sequences=False, input_shape=(n_timesteps, n_features)))
    # model.add(Dense(1))
...
def make_my_graph2(actual_train, train_predicted, actual_test, test_predicted, name):
    ...
    # '''QUICK DEBUG'''
    # print("\nActual Test Shape", actual_test.shape)
    # print("\nPredicted Test Shape", test_predicted.shape)
    '''Convert to Degrees'''
    ...
    train_predicted = train_predicted.reshape(train_predicted.shape[0], train_predicted.shape[1])
    ...
    soil_movement_horizontal = np.linspace(start=true_min-40, stop=true_max+20)
    ...
    pyplot.title("STACKED-LSTM")

    '''Save our figure'''
    pyplot.savefig('plots/STACKED_LSTM/' + name + '.png')
...
def build_model(train, number_of_lags, number_of_prediction_step, take_old_model = True):
    ...
    # print("Insight in training ", train_x[5], "Test", train_y[5], np.shape(train_x))
    # exit()

    #DEBUG
    print("DEBUG", train_x.shape[1], train_x.shape[2], train_x.shape)
    print("Insight in training ", train_x[5], "Test", train_y[5])

    # train_x = train_x.reshape(train_x[0], number_of_lags, 3)
...
def evaluate_model(train_dict, test_dict, number_of_lags, number_of_prediction_step):
    ...
    for sensor_id in test_dict:
        ...
        score_, scores_ = evaluate_forecasts(test_data, prediction_test)

        train_data = train_dict[sensor_id][number_of_lags:len(train_dict[sensor_id]) - number_of_prediction_step][:, :,
                     0]
        prediction_train = np.array(predictions_train[sensor_id])

        score_train_, scores_train_ = evaluate_forecasts(train_data, prediction_train)
